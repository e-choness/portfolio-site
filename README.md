...
Visits are counted with [GoatCounter](https://www.goatcounter.com) (no cookies, no personal data) and shown in the OS: the **Stats** app, the scrolling **telemetry panel** on the desktop (`ticker.js`), read counts in the Blog window, and the `stats` terminal command.

- **Counting** — `assets/js/echoos/analytics.js` loads GoatCounter's script only when `_config.yml` sets `goatcounter:` (production; `_config_dev.yml` blanks it) and the browser doesn't send Do Not Track / Global Privacy Control. The OS reports desktop loads, posts (`/blog/<slug>`), projects (`/projects/<slug>`), 404s, and events for windows (`app:<id>`), games (`game:<id>`) and the resume.
- **Aggregates** — `scripts/fetch-stats.mjs` runs in the deploy workflow (nightly at 07:17 UTC and on every push) with the `GOATCOUNTER_TOKEN` repository secret and writes `assets/data/stats.json` (git-ignored). Without the token it skips, and the OS shows its "pending" state.
- **Live counts** — the per-post count in the reader and the all-time tile use GoatCounter's public counter, which needs *Settings → "Allow adding visitor counts on your website"* enabled.
- **Site facts** (posts, projects, words, diagrams, commits) are computed at build time into `content.json` → `site`.
