...
Read that diagram top to bottom and the dependency direction is the whole argument: interfaces depend on the runtime, the runtime depends on the kernel, the kernel depends on the contracts, and the governance features at the bottom point *up* at the contracts too, exactly like any third-party plugin would. Nothing in the core points down at them.
...
- **Conditional routing:** visible structure rather than nested branches. The residency branch in that diagram is a real edge, not buried logic.
- **Checkpointing:** runs persist and survive a server restart.
- **Human-in-the-loop:** a guardrail can *pause* a request via an interrupt instead of only blocking it, and the run resumes later.
...
One detail that made the plugin model actually composable: nodes never call each other. They communicate only through a typed `RunState` that flows through the graph: the run id, the authenticated principal, the messages, a free-form `labels` dictionary, a mask map kept in a channel that is never serialized into model-visible text, the selected route, an append-only event log, and usage accumulators.
...
[The next post]({% post_url 2026-05-20-guardrails-that-tell-you-the-truth %}) is about the single hardest part of the build: designing a guardrail contract that is configurable, composable, *and* honest about what it can guarantee, especially when streaming and full-context scanning want exactly opposite things.
